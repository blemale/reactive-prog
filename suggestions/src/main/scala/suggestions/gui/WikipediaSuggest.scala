...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.sanitized.concatRecovered(wikiSuggestResponseStream(_).timedOut(10))
    ...
    val selections: Observable[String] =
      button.clicks.map(b => suggestionList.selection.items.headOption).filter(_.isDefined).map(_.get)

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.sanitized.concatRecovered(wikiPageResponseStream(_).timedOut(10))
